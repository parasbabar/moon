...
Verification is **only available on-chain**: it requires both a connected
Midnight wallet and a deployed contract. If the wallet is not connected or no
contract is available, the app clearly reports that state instead of falling
...
For the frontend to verify against this contract at build time, set
`VITE_CONTRACT_ADDRESS` to the address above (see
...
- **▶ Watch online**: [1-minute demo video (Google Drive)](https://docs.google.com/videos/d/1bnoz3R2g8axL68iaJaL-rk-JVz8Y9x6C1MD3XT8sN8Y/play?usp=sharing)
- **Download**: [demo-video.mp4](demo-video.mp4)

The video shows the complete privacy-preserving flow:
...
- **Proof generation in the wallet**: ZK proofs are generated by the connected
  Midnight wallet (Lace / I AM Wallet) during deployment and verification. No
  separate proof server is required for the browser flow.
